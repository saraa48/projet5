...
class RestaurantAccount extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        "http://django-env.zjepgtqmt4.us-west-2.elasticbeanstalk.com/api/restaurants/" +
    ...
    axios
      .get(
        "http://django-env.zjepgtqmt4.us-west-2.elasticbeanstalk.com/api/users/" +
    ...
    return (
      <div className="restaurantAccount">
        <Navbar>
          ...
          <Navbar.Brand href="/restaurant">UrbainFoods</Navbar.Brand>
          <Navbar.Toggle />
        ...
        <div className="accountInfo">
          ...
          <Form>
            ...
            <Button onClick={this.updateUserInfo} fluid color="black">
              Save
            </Button>
          </Form>
        </div>
...
class PastEvent extends Component {
  ...
  rateActivate(event, data) {
    ...
    axios
      .put(url + "/api/review/" + data.value + "/", info)
      .then(res => {
        window.location = `${process.env.PUBLIC_URL}/userInfo`;
      })
      .catch(err => {
  ...
  componentDidMount() {
    var pastCom = [];
    axios
      .get(url + "/api/review/?user=" + localStorage.getItem("id"))
      .then(res => {
        for (let i = 0; i < res.data.length; i++) {
          axios
            .get(url + "/api/restaurants/" + res.data[i].restaurant + "/")
            .then(response => {
              pastCom.push(
                <List.Item>
                  <List.Content>
...
class CommonAccount extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        "http://django-env.zjepgtqmt4.us-west-2.elasticbeanstalk.com/api/reservation/?user=" +
          localStorage.getItem("id")
      )
      .then(res => {
            ...
            .then(res1 => {
              componentList.push(
                <List.Item key={tempEvent[i].id}>
                  ...
                  <List.Content>
                    <List.Header>Restaurant Name: {res1.data.name}</List.Header>
                    <List.Description>
                  ...
                  <List.Content>
                    <List.Header>
                      Reservation Time: {tempEvent[i].dateTime.slice(11, 19)}
                    </List.Header>
                    <List.Description>
                      Phone Number: {res1.data.phone_num}
                    </List.Description>
                  </List.Content>
                  <List.Content>
                    <List.Header>Name: {tempEvent[i].first_name}</List.Header>
                    <List.Description>
  ...
  render() {
    const panes = [
      ...
      {
        menuItem: "Past Reservation",
        render: () => (
          <Tab.Pane>
            <PastEvent user={this.props.use} />
          </Tab.Pane>
        )
      },
      {
        menuItem: "Upcoming Reservation",
        render: () => (
          <Tab.Pane>
            <UpcomingEvent upcoming={this.state.upcomingEvent} />
          </Tab.Pane>
        )
      }
    ];
    return (
      <div className="commonAccount">
        <Navbar>
          ...
          <Navbar.Brand href="/">UrbainFoods</Navbar.Brand>
          <Navbar.Toggle />
          <Navbar.Collapse className="justify-content-end">
            <Dropdown trigger={this.props.trigger} className="icon" pointing>
              <Dropdown.Menu>
